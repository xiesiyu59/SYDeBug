...
/** 根据下标值添加 badge 未读数*/
- (void)addBadgeForIndex:(NSInteger)index num:(NSInteger)num {
    ...
    badge.tag = 3018 + index;
    UILabel *numLabel = [[UILabel alloc] init];
    numLabel.frame = CGRectMake(0, 0, badgeWidth, badgeHeight);
 ...
 *  @param index    标题所对应的下标
 */
- (void)resetColor:(UIColor *)color forIndex:(NSInteger)index {
    UIButton *button = (UIButton *)self.btnMArr[index];
 ...
 *  @param index    标题所对应的下标
 */
- (void)resetSelectColor:(UIColor *)color forIndex:(NSInteger)index {
    UIButton *button = (UIButton *)self.btnMArr[index];
