...
- (void)viewDidLoad {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        for (NSInteger i = 0; i < self->_fetchResult.count; i++) {
            PHAsset *asset = [self->_fetchResult objectAtIndex:i];
            [self->_imageManager requestImageDataForAsset:asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
                UIImage *imageOrigina = [UIImage imageWithData:imageData];
                [self.imageArray addObject:imageOrigina];
            }];
        }
...
- (void)initWithinitializationDataSource {
    ...
    if (!_fetchResult) {
        ...
        switch (self.type) {
            ...
            case PhotoTypeImage:{
                _fetchResult = [PHAsset fetchAssetsWithMediaType:PHAssetMediaTypeImage options:allPhotosOptions];
            }break;
            case PhotoTypeVideo:{
                _fetchResult = [PHAsset fetchAssetsWithMediaType:PHAssetMediaTypeVideo options:allPhotosOptions];
            }break;
            default:break;
        }
    }
...
//    SystemPhotoDetailViewController *vc = [[SystemPhotoDetailViewController alloc] init];
//    vc.imageArray = [self.imageArray copy];
//    [self.navigationController pushViewController:vc animated:YES];
    
    
    GridViewCell *cell = (GridViewCell *)[collectionView cellForItemAtIndexPath:indexPath];
    [SYPhotoBrowserVc showWithDataSource:[self.imageArray copy] currentIndex:indexPath.row imageView:cell.imageView type:SYPhotoBrowserTypeImage currentVc:self];
}
